...
function inlineDocumentWithCb($, css, callback) {
  try {
    inlineDocument($, css);
    callback();
  } catch (err) {
...
function getStylesData($, options, callback) {
  ...
  for (i = 0; i < stylesList.length; ++i) {
    ...
    if (options.removeStyleTags) $(styleElement).remove();
  }
...
function getStylesheetList($, options) {
  var results = [];
  var linkList = $("link");
